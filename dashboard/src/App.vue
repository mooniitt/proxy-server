<template>
  ...
  <div class="main-container">
    ...
    <div class="sidebar" :style="{ width: sidebarWidth + 'px' }">
      ...
      <TrafficMonitor v-if="activeTab === 'traffic'"
                      :traffic="traffic"
    ...
    <div class="editor-container">
      <RuleEditor v-if="editorMode === 'rule' && currentRule" 
                  :rule="currentRule"
                  @delete="deleteRule"
                  @save="saveAll" />
      
      <TrafficDetail v-else-if="editorMode === 'traffic' && selectedLog"
                     :log="selectedLog"
...
const doResize = (e) => sidebarWidth.value = Math.max(250, Math.min(600, e.clientX))
const stopResizing = () => {
...
const getTraffic = async (showMessage = false, silent = false) => {
    try {
        const res = await axios.get('/api/traffic', { params: { silent }})
        traffic.value = res.data || []
        if (showMessage && traffic.value.length > 0) showToast(`已加载 ${traffic.value.length} 条流量记录`)
    } catch (e) { if(showMessage) showToast('获取流量数据失败', 'error') }
}
